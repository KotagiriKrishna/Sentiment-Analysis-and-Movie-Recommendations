...
function recommendMovie() {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            //fetching json data response
            const jsonData = JSON.parse(xhr.response)

            console.log(jsonData);

            //movies from jsondata
            const movies = jsonData.result.movies;

            //refreshing page when new input is given

            document.getElementById("movies").innerHTML ="";
          

            var movies_list = document.getElementById("movies");
            

            for(let i =0 ; i< movies.length ;i++){

                //createProjectCardimg(movies[i]);

                var name = document.createElement('p');
                name.textContent = movies[i];
                movies_list.appendChild(name);
...
function getSentiment() {
    ...
    var input_review = document.getElementById("input_review").value;

    var xhr = new XMLHttpRequest();

    xhr.open("POST", "/getsentiment", true);
    xhr.setRequestHeader("Content-Type", "application/json");
    ...
    xhr.onreadystatechange = function () {
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            // sentiments from jsondata

            const sentiments = jsonData.result.sentiment;
            ...
            res.textContent = sentiments;
            reviews.appendChild(res);
 
             
     }
};

xhr.send(JSON.stringify({ text: input_review }));
}
...
// Display filtered movies in the autocomplete list
filteredMovies.forEach(function (movie) {
    var listItem = document.createElement("li");
    listItem.textContent = movie;
    listItem.addEventListener("click", function () {
        input.value = movie;
        suggestionList.innerHTML = ""; // Clear autocomplete list after selection
    });
...
//function to autocomplete the input movie name 
var moviesList = ["Avatar" ,
...
/*
fetch('C:\Users\krishna\OneDrive\Desktop\Movie Recommendations\static\js\movies.json').then(response =>
response.json())
.then(data=>{
    moviesList = data;
